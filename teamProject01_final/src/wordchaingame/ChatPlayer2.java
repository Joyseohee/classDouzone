...
public class ChatPlayer2 {
	...
	public static void main(String[] args) {
		...
		// 로그인 제작시 여기서 합치면
		System.out.println("solo play=1 , multiplay = 100");
		int n = sc.nextInt();
		if (n == 1) {
			try {
				Soloplay s = new Soloplay();
				Soloplay.main(args);
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
			System.out.println("게임 시작 (단어) 혹은 저장(save)");
			// 1번
			...
			try {
				...
				// 국어사전에 등재된 단어인지
				GAMEDICController.setConn(KRDICController3.getConn()); // 이 게임내에서 중복되는 단어 사용 방지 ex)사자 ->자사-> 사자-> 자사

				// String ip = "192.168.110.3"; // ip값 초기화
				String ip = "172.30.1.48"; // ip값 초기화

				Socket s = new Socket(ip, 7778); // Socket 객체 생성
				...
				Thread sender = new Thread(new ChatPlayer2.ClientSender(s)); // thread객체 sender를 생성 : 생성된 객체 sender는
	...
	static class ClientSender extends Thread {
		...
		public ClientSender(Socket s) { // 생성자 함수
			this.s = s;
			try {
		...
		public void run() { // override 된것을 확인할 수 있다. 이는 Runnable interface의 run 함수를 오버라이딩한 것이다.
			...
			try {
				...
				while (dos != null) {
					...
					// 입력
					String a = sc.next();
					if (a.equalsIgnoreCase("save")) {
						// 로그인

						// UI 실행
						GameUI m = new GameUI();
						m.Main1();
						// ui창이 열리고 로그인이 될때까지 추가 진행을 멈추는 반복문
						// boolean 으로 코딩하였으나 회원가입후 로그인시 저장이 되지않는 문제를 수정하기위해 바로 아이디를 불러오는것으로 변경
						// 수정 전 while (m.isLogincheck());
						while (m.getid().equals("1")) {
							System.out.println();
						}

						String uid = m.getid();
						// 유저에게 줄 점수
						System.out.println(KRDICController3.getscore(uid));
						String s = KRDICController3.getscore(uid);
						Point += Integer.parseInt(s);// 유저 아이디에서 점수 받아와서 더해주기
						String point = Integer.toString(Point); // 점수를 스트링으로 변환 데베에 넣기 위해
						KRDICController3.setscore(point, uid);
						String rank = KRDICController3.getRank(uid);

						System.out.println("uid: " + uid + ", point: " + point + ", rank: " + rank);
						System.out.println("종료합니다.");
//						System.exit(0);

					} else if (word != null) {
						// 1글자는 실행 안됨(서버는 엔터를 입력으로 처리함)
						// 첫입력이 아니면(게임 진행중 이면)
						// 뒷말 앞말비교 + 사전에 있는 말인지+이번 게임에 사용된 단어가 아닌지
						if (a.length() != 1 && word.charAt(word.length() - 2) == a.charAt(0)
								&& KRDICController3.gamecheck(a) == true && GAMEDICController.gamecheck(a) == false) {
							dos.writeUTF(a);
							GAMEDICController.game(a);
						} else {
							if (a.length() == 1) {
								dos.writeUTF("#" + name + "] 단어: " + a + "는(은) 너무 짧은 문장 오류 lose");
							} else if (word.charAt(word.length() - 2) != a.charAt(0)) {
								dos.writeUTF("#" + name + "] 단어: " + a + "는(은) 끝말 오류 lose");
							} else if (KRDICController3.gamecheck(a) == false) {
			...
			while (dis != null) { // * 현재 슬레드는 지속적으로 상대방 서버로부터 데이터가 오는지 확인하고, 만약 데이터가 왔다면 println으로 읽은 값을 출력한다.
				try {
					...
					if (read.charAt(0) == '[' || read.charAt(0) == '#') {
						System.out.println("새로운 경기가 시작합니다.");
						word = null;
						GAMEDICController.truncateT();
						if (read.charAt(0) == '#')// 승리 취급
						{
							Point += 10;
							System.out.println("포인트" + Point + "점 획득 저장하려면 'save'를 입력하세요");
						}
					} else {
						word = read;
					}
