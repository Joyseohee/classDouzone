...
public class KRDICController3 { // controller
	...
	*/
	...
	// 유저의 점수를 가져오는 함수
	public static String getscore(String u_id) throws SQLException { // 예외처리 위임
		KRDICVO vo = new KRDICVO();
		Statement st = conn.createStatement();

		String word1 = "";
		ResultSet r = st.executeQuery("select score from guser where u_id = '" + u_id + "'");
		// 오답입니다.
		if (r.next() == false) {
			word1 = "null오류입니다";
			return word1;
	...
	//랭크를 가져오는 함수
	public static String getRank(String u_id) throws SQLException { // 예외처리 위임
		KRDICVO vo = new KRDICVO();
		Statement st = conn.createStatement();

		String word1 = "";
		ResultSet r = st.executeQuery("select RANK from guser where u_id = '" + u_id + "'");
		// 오답입니다.
		if (r.next() == false) {
			word1 = "null오류입니다";
			return word1;
	...
	//점수 저장+ 점수에 따른 랭크 수정(반드시 점수를 받아온 이후 사용할것)
	public static void setscore(String point,String id) throws SQLException { // 예외처리 위임
		KRDICVO vo = new KRDICVO();
		Statement st = conn.createStatement();

		st.executeQuery("update guser set score="+ point+" where u_id = '"+ id +"'");
		//저장되었습니다
		System.out.println("저장되었습니다.");
		//point 기본값은 브론즈
		int rank = Integer.parseInt(point);
		if (100<=rank&&rank<500)
			st.executeQuery("update guser set Rank = 'SILVER' where u_id = '"+ id +"'");
		else if (500<=rank&&rank<1000)
			st.executeQuery("update guser set Rank = 'GOLD' where u_id = '"+ id +"'");
		else if (1000<=rank)
			st.executeQuery("update guser set Rank = 'PLATINUM' where u_id = '"+ id +"'");

		st.executeQuery("commit");
	}

	// 게임진행 문자열 받으면 검색해서 유무 알려줌
