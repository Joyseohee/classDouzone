...
public class GAMEDICController {
	...
	//없으면 false 있으면 true
	public static boolean gamecheck(String word) throws SQLException {
		connect();
		Statement st = conn.createStatement();
		ResultSet r = st.executeQuery("SELECT * FROM GAME1 where GWORD = '" + word + "'");
		if (r.next() == true) {
			return true;
		} else { // 중복 있음
			return false;
	...
	public static void createT() throws SQLException {
		...
		try {
			sql = "CREATE TABLE \"GAME\" (\n" + "    GWORD VARCHAR2(30)\n"
					+ "        CONSTRAINT \"GW_NULL\" CHECK(\"GWORD\" IS NOT NULL)\n"
