...
public class ChatPlayer {
    ...
    public static void main(String[] args) {
        ...
        try {
             ...
             *
             * 소켓이 생성될 때, 소켓에 I/O작업을 할 수 있는 InputStream, OutputStream도 같이 생성된다.
             * 싱글 스레드에서 받은 데이터가 있는지 확인하기 위해 while문을 사용할 경우
             * 연결된 상대방이 보낸 데이터를 읽을때까지, 프로그램의 흐름이 멈춘다.
             * 따라서 상대방이 보낸 데이터를 읽는 스레드와 메세지를 입력받아 상대방에게 전송하는 스레드를 분리하여 구현하면,
             *  상대방의 데이터를 읽을때까지 프로그램이 멈춰있지 않게된다.
             *
             */
            ...
            Thread sender = new Thread(new ClientSender(s, args[0]));   // thread객체 sender를 생성 : 생성된 객체 sender는 ClientSender클래스 객체이다. 여기서 2번을 확인한다.
    ...
    static class ClientSender extends Thread{ // inner class
        ...
        public ClientSender(Socket s, String name) { // 생성자 함수
            this.s = s;
            try {
                dos = new DataOutputStream(s.getOutputStream());  //Socket의 outputstream을 할당받는다. 즉 Socket에서 Data를 전송할 수 있는 방법을 가져온다.
        ...
        public void run() { // override 된것을 확인할 수 있다. 이는 Runnable interface의 run 함수를 오버라이딩한 것이다.
            // ClientSender는 thread클래스를 확장했기 때문에 부모 클래스인 thread클래스가 implements한 Runnable 인터페이스를 사용할 수 있기 때문이다. (자식 클래스는 부모 클래스의 모든 기능을 상속받는다.)
            Scanner sc = new Scanner(System.in);
            try {

                if(dos != null) dos.writeUTF(name);// thread를 시작했을 때, 소켓의 outputstream이 열려있었다면, args로 입력받았던 name값을 전달한다.
                while(dos!=null)
                	{
