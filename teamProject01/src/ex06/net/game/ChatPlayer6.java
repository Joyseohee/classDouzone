...
public class ChatPlayer6 {
	...
	public static void main(String[] args) {
		...
		// 로그인 제작시 여기서 합치면
		System.out.println("solo play=1 , multiplay = else (int)");
		int n = sc.nextInt();
		if (n == 1) {
			try {
			...
			try {
				...
				while (dos != null) {
					...
					if (a.equalsIgnoreCase("save")) {
						...
						String uid= m.getid();
						//유저에게 줄 점수
						System.out.println(KRDICController.getscore(uid));
						String s= KRDICController.getscore(uid);
						Point += Integer.parseInt(s);//유저 아이디에서 점수 받아와서 더해주기
						String point = Integer.toString(Point); //점수를 스트링으로 변환 데베에 넣기 위해
						KRDICController.setscore(point, uid);
						
						System.out.println("uid: "+uid+", point: "+point);
						...
						if (a.length() != 1 && word.charAt(word.length() - 2) == a.charAt(0)
								&& KRDICController.gamecheck(a) == true && GAMEDICController.gamecheck(a) == false) {
							dos.writeUTF(a);
